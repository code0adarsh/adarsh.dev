...
// Custom class interfaces to fix TypeScript errors
...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement): { gl: WebGLRenderingContext; ext: Ext } {
      const params = {
        alpha: true,
        depth: false,
        stencil: false,
      ...
      const halfFloatTexType = isWebGL2
        ...
        : halfFloat ? (halfFloat as any).HALF_FLOAT_OES : null;
    ...
    function resizeDoubleFBO(
      ...
      let newWrite = createFBO(w, h, internalFormat, format, type, param);
      
      return {
        width: w,
        height: h,
        texelSizeX: newRead.texelSizeX,
        texelSizeY: newRead.texelSizeY,
        read: newRead,
        write: newWrite,
        swap: target.swap
      };
    ...
    function updatePointerMoveData(pointer: PointerPrototype, posX: number, posY: number, color: any) {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width;
      pointer.texcoordY = 1.0 - posY / canvas.height;
